...
class Orders_Fake {
    ...
    public function get_custom_first_names() {
        $names_string = get_option( 'razzi_live_sales_notification_name' );

        if( empty( $names_string ) ) {
            $names_string = esc_html__( 'Someone', 'razzi' );
        }
    ...
    public function get_custom_location() {
        $location_string = get_option( 'razzi_live_sales_notification_location' );
        $location = '';
    ...
    public function set_time() {
        $time_type  = get_option( 'razzi_live_sales_notification_time_type', 'day' );
        $time       = get_option( 'razzi_live_sales_notification_time', 1 );
        $order_time = '-' . $time .' '. $time_type;
        $present 	= time();

        $till = strtotime( $order_time ); // 1 day from present time
