...
class Posts_Carousel_2 extends Widget_Base {
	...
	protected function render() {
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			$date_html = $settings['date_format_select'] == 'custom' ? '<div class="blog-date">' . get_the_date( $settings['date_format'] ) . '</div>' : $date_html;
			...
			$html[] = '<'.$settings['title_size'].' class="entry-title"><a href="' . $post_url['url'] . '">' . get_the_title( get_the_ID() ) . '</a></'.$settings['title_size'].'>';
