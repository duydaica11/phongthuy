...
class Posts_Carousel extends Widget_Base {
	...
	protected function section_content_style() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .razzi-posts-carousel .post-meta a' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .razzi-posts-carousel .post-title a' => 'color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		while ($query->have_posts()) : $query->the_post();
			...
			$date_format = $settings['date_format_select'] == 'custom' ? $settings['date_format'] : 'M d, Y';
			$date_html   = ' / <span class="meta-date">' . esc_html( get_the_date( $date_format ) ) . '</span>';
			...
			$html[] = $date_html;
			$html[] = '</div>';
			$html[] = '<'.$settings['title_size'].' class="post-title"><a href="'.$post_url['url'].'">'.get_the_title( get_the_ID() ).'</a></'.$settings['title_size'].'>';
			$html[] = '</div>';
			$html[] = '</article>';
