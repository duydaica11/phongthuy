...
<div class="yith-wcwl-add-button">
	...
	<a href="#add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" rel="nofollow" class="rz-loop_button <?php echo esc_attr( $link_classes ); ?> open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>]" data-text="<?php echo esc_attr( apply_filters( 'yith_wcwl_add_to_wishlist_title', $label ) ); ?>">
		<?php echo ! empty( $icon ) ? $icon : '';?>
		<span class="loop_button-text"><?php echo wp_kses_post( $label ); ?></span>
	</a>
			...
			<div class="yith-wcwl-popup-content">
				...
				<?php if ( apply_filters( 'yith_wcwl_show_popup_heading_icon_instead_of_title', ! empty( $heading_icon ), $heading_icon ) ) : ?>
					<p class="heading-icon">
						<?php echo yith_wcwl_kses_icon( apply_filters( 'yith_wcwl_popup_heading_icon_class', $heading_icon ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</p>
				<?php else : ?>
				...
				<p class="popup-description">
					<a href="#" class="show-tab active" data-tab="choose"><?php echo esc_html( apply_filters( 'yith_wcwl_choose_wishlist_text', __( 'Choose a wishlist', 'razzi' ) ) ); ?></a>
					<?php esc_html_e( 'or', 'razzi' ); ?>
					<a href="#" class="show-tab" data-tab="create"><?php esc_html_e( 'Create a new list', 'razzi' ); ?></a>
				</p>
						...
						<p class="form-row form-row-wide">
							<select name="wishlist_id" class="wishlist-select">
								...
								<?php else : ?>
									...
									<?php endforeach; ?>
								<?php endif; ?>
								<option value="new"><?php echo esc_html( apply_filters( 'yith_wcwl_create_new_list_text', __( 'Create a new list', 'razzi' ) ) ); ?></option>
							</select>
						</p>
					</div>
				</div>
				...
				<div class="tab create">
					<p class="form-row form-row-wide">
						<input name="wishlist_name" class="wishlist-name input-text" type="text" placeholder="<?php echo esc_html( apply_filters( 'yith_wcwl_new_list_title_text', __( 'Enter wishlist name', 'razzi' ) ) ); ?>"/>
					</p>
