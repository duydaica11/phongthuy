...
class Topbar {
	...
	public function topbar_items( $items ) {
		...
		foreach ( $items as $item ) {
			...
			switch ( $item['item'] ) {
				...
				case 'text-2':
					$html_svg = '';
					if ( $svg = Helper::get_option( 'topbar_svg_code_2' ) ) {
						$html_svg = '<span class="razzi-svg-icon">' . \Razzi\Icon::sanitize_svg( $svg ) . '</span>';
					}

					echo '<div class="razzi-topbar__text">' . $html_svg . do_shortcode( wp_kses_post( Helper::get_option( 'topbar_text_2' ) ) ) . '</div>';
	...
	public function topbar_items_option() {
		return apply_filters( 'razzi_topbar_items_option', array(
			...
			'text-2'     		=> esc_html__('Custom Text 2', 'razzi'),
