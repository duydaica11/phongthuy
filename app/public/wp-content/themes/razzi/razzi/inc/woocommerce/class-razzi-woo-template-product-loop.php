...
class Product_Loop {
	...
	public function product_loop_content_hooks() {
		...
		if( Helper::get_option('product_loop_empty_rating') ) {
			add_filter('woocommerce_product_get_rating_html', array( $this, 'product_get_rating_html' ), 20, 3);
		}
	...
	public function loop_start( $html ) {
		...
		$alignment_class = 'product-loop-center';
		if ( in_array( $loop_layout, array( '10' ) ) ) {
			$alignment_class = Helper::get_option( 'product_loop_alignment' ) == 'left' ? 'product-loop-left' : $alignment_class;
		}

		$classes[] = in_array( $loop_layout, array( '2', '3', '4', '5', '6', '7', '10', '12' ) ) ? $alignment_class : '';

		if ( in_array( $loop_layout, array( '2', '3', '9' ) ) ) {
			$classes[] = intval( Helper::get_option( 'product_loop_wishlist' ) ) ? 'show-wishlist' : '';
			$classes[] = intval( Helper::get_option( 'product_loop_compare' ) ) ? 'show-compare' : '';
		}
		...
		if ( $mobile_pl_col = intval( Helper::get_option( 'mobile_landscape_product_columns' ) ) ) {
			$mobile_pl_col = intval( wc_get_loop_prop( 'columns' ) ) !== 1 ? $mobile_pl_col : '1';
			$classes[] = 'mobile-pl-col-' . $mobile_pl_col;
		}

		if ( $mobile_pp_col = intval( Helper::get_option( 'mobile_portrait_product_columns' ) ) ) {
			$mobile_pp_col = intval( wc_get_loop_prop( 'columns' ) ) !== 1 ? $mobile_pp_col : '1';
	...
	public function product_get_rating_html($html, $rating, $count) {
		if ( 1 > $rating ) {
			...
			$label = sprintf( __( 'Rated %s out of 5', 'razzi' ), $rating );
