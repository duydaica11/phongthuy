...
class Dynamic_CSS {
	...
	public function product_badges_static_css() {
		...
		if ( ( $bg = Helper::get_option( 'shop_badge_custom_bg' ) ) && $bg != '' ) {
			$static_css .= '.woocommerce-badges .custom {background-color: ' . $bg . '}';
		}

		if ( ( $color = Helper::get_option( 'shop_badge_custom_color' ) ) && $color != '' ) {
			$static_css .= '.woocommerce-badges .custom {color: ' . $color . '}';
		}
