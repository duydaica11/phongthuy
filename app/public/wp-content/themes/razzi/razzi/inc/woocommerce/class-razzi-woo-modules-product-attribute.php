...
class Product_Attribute {
	...
	public function __construct() {
		if( in_array(Helper::get_option( 'product_loop_layout' ), array( '10' ) ) ) {
			add_action( 'woocommerce_shop_loop_item_title', array( $this, 'product_attribute' ), 11 );
	...
	public function product_attribute() {
		...
		$product_attribute_number = empty($attrs_number) ? Helper::get_option( 'product_loop_attribute_number' ) : $attrs_number;
		...
		if ( ! $product_attribute['is_taxonomy'] ) {
			...
			if ( isset($product_attribute['is_variation']) && $product_attribute['is_variation'] ) {
				...
				foreach( $available_variations as $variation ) {
					...
					if( $index >= $product_attribute_number ) {
						continue;
					}
				...
				if( $index >= $product_attribute_number ) {
					$count_more = count($variations_atts) - $index;
