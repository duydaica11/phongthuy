...
echo apply_filters(
	...
	sprintf(
		...
		esc_attr( isset( $args['aria-label'] ) ? $args['aria-label'] : sprintf( __( 'Compare %s', 'razzi' ), '&ldquo;' . $product->get_title() . '&rdquo;' ) ),
		__( 'Compare', 'razzi' ),
		...
		esc_html( isset( $args['label'] ) ? $args['label'] : __( 'Compare', 'razzi' ) )
	),
	$args
);
