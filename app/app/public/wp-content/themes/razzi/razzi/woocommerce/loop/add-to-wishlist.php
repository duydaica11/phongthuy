...
echo apply_filters(
	...
	sprintf(
		'<a href="%s" data-quantity="%s" data-product_id="%d" data-variations="%s" class="%s" aria-label="%s" data-text="%s" data-product-title="%s">
			%s
		...
		esc_attr( isset( $args['aria-label'] ) ? $args['aria-label'] :sprintf( __( 'Add %s to the wishlist', 'razzi' ), '&ldquo;' . $product->get_title() . '&rdquo;' ) ),
		__( 'Wishlist', 'razzi' ),
		...
		esc_html( isset( $args['label'] ) ? $args['label'] : __( 'Add to wishlist', 'razzi' ) )
	),
	$args
);
