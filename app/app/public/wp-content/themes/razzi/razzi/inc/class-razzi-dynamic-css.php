...
class Dynamic_CSS {
	...
	protected function page_static_css() {
		...
		if ( $rz_department_menu_display == 'onpageload' && $rz_department_menu_display_spacing ) {
			...
			$static_css .= '.site-header:not(.minimized) .header-items .header-department.show_menu_department .department-menu .nav-menu > li > .dropdown-submenu { top: ' . intval( $rz_department_menu_display_spacing ) . 'px; }';
		}
	...
	protected function header_static_css() {
		...
		// Header Compare
		if ( intval( Helper::get_option( 'header_compare_counter' ) ) && intval( Helper::get_option( 'header_compare_custom_color' ) ) ) {
			$header_compare_bg           = Helper::get_option( 'header_compare_background_color' );
			$header_compare_text_color   = Helper::get_option( 'header_compare_text_color' );

			if ( $header_compare_bg ) {
				$static_css .= '.header-compare .counter { --rz-background-color-primary: ' . esc_attr( $header_compare_bg ) . ' }';
			}

			if ( $header_compare_text_color ) {
				$static_css .= '.header-compare .counter { --rz-background-text-color-primary: ' . esc_attr( $header_compare_text_color ) . ' }';
			}
		}
		...
		if ( intval( Helper::get_option( 'header_search_custom_color' ) ) ) {
			...
			$header_search_text_placeholder_color   = Helper::get_option( 'header_search_text_placeholder_color' );
			...
			if ( $header_search_bg ) {
				$static_css .= '#site-header .header-search.search-form-type .search-field { background-color: ' . esc_attr( $header_search_bg ) . '}';
				$static_css .= '#site-header .header-search.search-form-type .product-cat-label { background-color: ' . esc_attr( $header_search_bg ) . '}';
			...
			if ( $header_search_text_color ) {
				$static_css .= '#site-header .header-search.form-type-boxed .search-field, #site-header .aws-container .aws-search-form .aws-search-field { --rz-color-placeholder: ' . esc_attr( $header_search_text_color ) . ' }';
				$static_css .= '#site-header .header-search .search-field { --rz-header-color-darker: ' . esc_attr( $header_search_text_color ) . ' }';
				$static_css .= '#site-header .header-search.search-type-form-cat .product-cat-label { --rz-header-text-color-gray: ' . esc_attr( $header_search_text_color ) . ' }';
				$static_css .= '#site-header .aws-container .aws-search-form .aws-search-field { color: ' . esc_attr( $header_search_text_color ) . ' }';
			}

			if ( $header_search_text_placeholder_color ) {
			...
			if ( $header_search_border_color ) {
				$static_css .= '#site-header .header-search .form-search, #site-header .aws-container .aws-search-form .aws-search-field  { border-color: ' . esc_attr( $header_search_border_color ) . ';
				...
				$static_css .= '#site-header .site-header .header-search.form-type-boxed .search-field { border-color: ' . esc_attr( $header_search_border_color ) . ';}';
			}

			if ( $header_search_border_hover ) {
				$static_css .= '#site-header .header-search.search-type-form-cat .search-field:focus, #site-header .aws-container .aws-search-form .aws-search-field:focus { border-color: ' . esc_attr( $header_search_border_hover ) . ';
														--rz-border-color-dark: ' . esc_attr( $header_search_border_hover ) . ';}';
				$static_css .= '#site-header .header-search .border-color-dark { --rz-border-color-dark: ' . esc_attr( $header_search_border_hover ) . ';}';
			}

			if ( $header_search_border_item_color ) {
				$static_css .= '#site-header .site-header .header-search.search-type-form-cat .product-cat-label { border-left-color: ' . esc_attr( $header_search_border_item_color ) . ';
																									border-right-color: ' . esc_attr( $header_search_border_item_color ) . '}';
				$static_css .= '#site-header .site-header .header-search.search-type-form-cat .search-submit { border-right-color: ' . esc_attr( $header_search_border_item_color ) . ';}';
				$static_css .= '#site-header .border-color-dark { --rz-border-color-dark: ' . esc_attr( $header_search_border_item_color ) . ' !important;}';
			}
		...
		if ( intval( Helper::get_option( 'header_department_custom_color' ) ) ) {
			...
			$header_department_bg_gradient  = Helper::get_option( 'header_department_background_gradient_color' );
			...
			if ( $header_department_bg_gradient ) {
				$static_css .= '.header-department { background: linear-gradient(92.37deg, '. $header_department_bg .' 0%, '. $header_department_bg_gradient .' 100%) }';
			} else {
			...
			if ( $header_department_border_color ) {
				$static_css .= '#site-header .header-department { border-color: ' . esc_attr( $header_department_border_color ) . ' }';
			}
		}
	...
	public function mobile_header_static_css() {
		...
		if ( ! empty( $dimension ) ) {
			$logo_width = isset($dimension['width']) ? $dimension['width'] : 'auto';
			$logo_height = isset($dimension['height']) ? $dimension['height'] : 'auto';
			$logo_width =  is_numeric( $logo_width ) ? $logo_width  .'px' : $logo_width;
			$style = $logo_width ? 'width: ' . $logo_width . ';' : '';
			$logo_height =  is_numeric( $logo_height ) ? $logo_height  .'px' : $logo_height;
			$style .= $logo_height ? 'height: ' . $logo_height . ';' : '';
