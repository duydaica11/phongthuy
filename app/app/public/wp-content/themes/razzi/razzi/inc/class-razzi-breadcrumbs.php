...
class Breadcrumbs {
	...
	public function breadcrumbs( $args = array() ) {
		...
		if (function_exists('rank_math_the_breadcrumbs') && class_exists('\RankMath\Helper') && \RankMath\Helper::is_breadcrumbs_enabled()) {
			...
			$atts['wrap_before'] = '<nav aria-label="breadcrumbs" class="rank-math-breadcrumb site-breadcrumb">';
			$atts['wrap_after'] = '</nav>';

			return  rank_math_the_breadcrumbs($atts);
		}
		...
		elseif ( is_page() ) {
			if ( ( function_exists( 'is_cart' ) && is_cart() ) || ( function_exists( 'is_checkout' ) && is_checkout() ) ) {
				...
				if ( $shop_page_id && $shop_page && isset( $permalinks['product_base'] ) && strstr( $permalinks['product_base'], '/' . $shop_page->post_name ) && intval( get_option( 'page_on_front' ) ) !== $shop_page_id ) {
					$items[] = sprintf( $item_tpl, esc_url( get_permalink( $shop_page_id ) ), get_the_title( $shop_page_id ) );
				}
